...
export default function AdminLoginDialog({ open, configured, onClose, onSuccess }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[90] grid place-items-center overflow-y-auto bg-brand-navy/55 p-4 backdrop-blur-sm" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div role="dialog" aria-modal="true" aria-labelledby="careers-login-title" className="relative w-full max-w-md overflow-hidden rounded-[1.6rem] border border-border bg-card p-6 shadow-2xl sm:p-8">
        <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-brand-blue/60 via-transparent to-brand-orange/60" />
        <button type="button" onClick={onClose} className="absolute right-4 top-4 grid size-10 place-items-center rounded-xl border border-border text-muted-foreground transition hover:bg-muted hover:text-foreground" aria-label="Close login">
          <X size={18} />
        </button>
        <span className="grid size-11 place-items-center rounded-xl bg-brand-blue/[0.08] text-brand-blue dark:text-brand-blue-light"><LockKeyhole size={20} /></span>
        ...
        <form onSubmit={submit} className="mt-6 space-y-5">
          ...
          {!configured && <div className="flex gap-2.5 rounded-xl border border-amber-500/25 bg-amber-500/8 p-4 text-sm text-amber-800 dark:text-amber-200"><AlertCircle size={18} className="mt-0.5 shrink-0" />Admin credentials are not configured on this environment.</div>}
